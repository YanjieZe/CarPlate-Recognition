...
class TemplateMatcher:
    def __init__(self, threshold=0.1, template_path='templates' ):
    ...
    def loop_match(self, plate_img):
        ...
        for data in template_dataset:
            label = data[0]
            template = data[1]
            result = self.match(plate_img, template)
            if result is None:
                continue
            else:
                (tH, tW) = template.shape[:2]
                ...
                if maxVal > self.threshold:
                    ...
                    result_template_list.append((label, (startX,startY)))
        ...
        def compare(a):
            return a[1][0]
        result_template_list.sort(key=compare)
...
if __name__=='__main__':
    ...
    test_template_path = 'templates/沪.png'
    ...
    plate_number = matcher.loop_match(test_img)
    print(plate_number)
